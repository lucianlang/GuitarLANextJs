import React from 'react';
import Image from 'next/image';
import Link from 'next/link';
import styles from '../styles/cartPopUp.module.css'


const CartPopUp = ({ cantidad, guitarra }) => {

    console.log(guitarra, cantidad)


    return (
        <div className={styles.popup}>
            <div className={styles.success}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={styles.checkIcon}>
                    <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clipRule="evenodd" />
                </svg>

                <p className={styles.agregoText}>Se agregó al carrito!</p>
            </div>
            <hr className={styles.divider}></hr>

            <div className={styles.toastContainer}>
                <Image
                    src={guitarra.imagen?.data.attributes.formats.medium.url}
                    alt={guitarra.nombre}
                    width={60}
                    height={60} />
                <div className={styles.textContainer}>
                    <h3 className={styles.nombre}>Guitarra - "{guitarra.nombre}"</h3>
                    <p className={styles.cantidad}>Cantidad: {cantidad}</p>

                </div>


            </div>
            <Link legacyBehavior href={`/carrito`}>
                <a className={`${styles.enlace} ver-producto`}>
                    Ver Carrito</a>
            </Link>
        </div>

    );
};

export default CartPopUp;
